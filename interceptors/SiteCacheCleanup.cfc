...
component extends="coldbox.system.Interceptor" {
	...
	function cbadmin_postEntrySave( event, interceptData ) {
		...
		if ( settings.feed_items_include_entries ) doCacheCleanup();
	}
	...
	function cbadmin_postPageSave( event, interceptData ) {
		...
		var settings = deserializeJSON( settingService.getSetting( "aggregator" ) );
		if ( settings.feed_items_entrypoint == page.getslug() || settings.feeds_entrypoint == page.getslug() ) {
			doCacheCleanup();
		}
	}
	...
	private SiteCacheCleanup function doCacheCleanup() {
		...
		// Announce event
		announceInterception( state="aggregator_onSiteCacheCleanup", async=true );
